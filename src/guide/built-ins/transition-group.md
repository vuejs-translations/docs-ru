...
Вы также можете задать пользовательские классы для анимации перемещаемого элемента, передавая свойство `moveClass` в `<TransitionGroup>`, аналогично [пользовательским классам анимации в `<Transition>`](/guide/built-ins/transition.html#custom-transition-classes).
