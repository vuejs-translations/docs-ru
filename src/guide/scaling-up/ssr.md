...
```js [app.js]
// (используется совместно сервером и клиентом)
import { createSSRApp } from 'vue'
...
```js [app.js]
// (совместно используется сервером и клиентом)
import { createSSRApp } from 'vue'
...
Библиотеки управления состояниями, такие как Pinia, разработаны с учетом этого. Более подробную информацию можно найти в [руководстве по SSR для Pinia.](https://pinia-ru.netlify.app/ssr/).
...
В Vue 3.5+ неизбежные несоответствия гидратации можно выборочно подавить с помощью атрибута [`data-allow-mismatch`](/api/ssr#data-allow-mismatch).
