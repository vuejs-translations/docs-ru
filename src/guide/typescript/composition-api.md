...
Это также работает, если `Props` импортируется из другого файла: относительный импорт, алиас пути (например, `@/types`) или внешняя зависимость (например, `node_modules`). Для этой возможности требуется, чтобы TypeScript был peer-зависимостью Vue.
