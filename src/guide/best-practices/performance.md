...
## Вырианты оценки {#profiling-options}
...
Если ваш сценарий использования чувствителен к производительности загрузки страницы, избегайте использования его в качестве чисто клиентского SPA. Вы хотите, чтобы ваш сервер напрямую передавал HTML, содержащий содержимое, которое хотят увидеть пользователи. Чисто клиентский рендеринг страдает от медленного времени перехода к содержимому. Его можно уменьшить с помощью [отрисовки на стороне сервера (SSR)](/guide/extras/ways-of-using-vue.html#fullstack-ssr) или [статической генерации сайта (SSG)](/guide/extras/ways-of-using-vue.html#jamstack-ssg). Ознакомьтесь с руководством [SSR Guide](/guide/scaling-up/ssr.html), чтобы узнать о выполнении SSR в Vue. Если ваше приложение не требует высокой интерактивности, вы также можете использовать традиционный внутренний сервер для рендеринга HTML и расширить его с помощью Vue на клиенте.
Если ваше основное приложение должно быть SPA, но в нем есть маркетинговые страницы (посадочные, о компании, блог), разворачивайие их отдельно! В идеале маркетинговые страницы должны быть развернуты как статический HTML с минимальным количеством JS с помощью SSG.

### Размер бандла и Tree-shaking {#bundle-size-and-tree-shaking}
...
Ленивую загрузку лучше всего использовать для функций, которые не требуются сразу после начальной загрузки страницы. В приложениях Vue это можно использовать в сочетании с функцией [Async Component](/guide/components/async.html) для создания разделенных кусков для деревьев компонентов:
...
`v-once` - это встроенная директива, которая может быть использована для вывода содержимого, зависящего от данных во время выполнения программы, но не требующего обновления. При этом все поддерево, для которого она используется, будет пропущено при всех последующих обновлениях. Более подробная информация приведена в [API reference](/api/built-in-directives.html#v-once).
