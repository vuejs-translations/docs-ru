...
Обратите внимание, что если `x` уже является ref, то `toRef(__temp, 'x')` просто вернёт его как есть, и дополнительный ref создан не будет. Если деструктурированное значение не является ref (например, функция), оно всё равно будет работать — значение будет обернуто в ref, так что остальная часть кода будет работать как положено.
