...
[Открыть в Template Explorer](https://template-explorer.vuejs.org/#eyJzcmMiOiI8ZGl2PlxuICA8ZGl2PmZvbzwvZGl2PiA8IS0tIGNhY2hlZCAtLT5cbiAgPGRpdj5iYXI8L2Rpdj4gPCEtLSBjYWNoZWQgLS0+XG4gIDxkaXY+e3sgZHluYW1pYyB9fTwvZGl2PlxuPC9kaXY+XG4iLCJvcHRpb25zIjp7ImhvaXN0U3RhdGljIjp0cnVlfX0=)

Элементы `foo` и `bar` статичны — пересоздавать vnode и сравнивать их при каждом повторном рендере не нужно. Рендерер создаёт эти vnode при первом рендере, кэширует их и повторно использует одни и те же vnode при последующих рендерах. Рендерер также может полностью пропустить их сравнение, когда видит, что старый и новый vnode — один и тот же.

Кроме того, при достаточном количестве подряд идущих статических элементов они объединяются в один «статический vnode», содержащий обычную HTML-строку для всех этих узлов ([пример](https://template-explorer.vuejs.org/#)). Такие статические vnode монтируются прямым присвоением `innerHTML`.
...
При повторном рендеринге компонента вместо полного дерева ему приходится обходить только сплющенное дерево. Это называется **Сплющивание дерева**, и значительно сокращает количество узлов, которые необходимо обходить при обходе виртуального DOM. При этом статические части шаблона фактически пропускаются.
...
- При гидратации необходимо обходить только блоковые узлы и их динамических потомков, что позволяет добиться частичной гидратации на уровне шаблона.
