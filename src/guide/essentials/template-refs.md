...
Хотя декларативная модель рендеринга Vue абстрагирует вас от большинства прямых операций с DOM, все же могут быть случаи, когда нам нужен прямой доступ к базовым элементам DOM. Для этого мы можем использовать специальный атрибут `ref`:

```vue-html
<input ref="input">
```

`ref` - это специальный атрибут, аналогичный атрибуту `key`, о котором говорилось в главе `v-for`. Он позволяет нам получить прямую ссылку на определенный элемент DOM или экземпляр дочернего компонента после его монтирования. Это может быть полезно, когда вы хотите, например, программно выставить фокус на поле ввода при монтировании компонента или инициализировать стороннюю библиотеку на элементе.
...
// вызываем функцию ref для создания ссылки
...
Обратите внимание, что доступ к ссылке возможен только **после того, как компонент будет смонтирован.** Если вы попытаетесь получить доступ к <span class="options-api">`$refs.input`</span><span class="composition-api">`input`</span> в выражении шаблона, то при первом рендере оно будет равно `null`. Это происходит потому, что элемент не существует до окончания первого рендеринга!

<div class="composition-api">

Если вы пытаетесь следить за изменениями ссылки на элемент шаблона, обязательно учитывайте случай, когда ссылка имеет значение `null`:
...
Когда `ref` используется внутри `v-for`, соответствующая ссылка должна содержать значение Array, которое будет заполнено элементами после монтирования:
...
Когда `ref` используется внутри `v-for`, результирующим значением ссылки будет массив, содержащий соответствующие элементы:
...
Вместо строкового ключа атрибут `ref` может быть привязан к функции, которая будет вызываться при каждом обновлении компонента и дает вам полную свободу выбора места хранения ссылки на элемент. Функция получает ссылку на элемент в качестве первого аргумента:
...
Обратите внимание, что мы используем динамическое связывание `:ref`, поэтому мы можем передать ему функцию вместо строки с названием ссылки. Когда элемент будет размонтирован, аргументом будет `null`. Конечно, вы можете использовать метод вместо встроенной функции.

## Ссылка на компонент {#ref-on-component}

> Этот раздел предполагает знание [Компоненты](/guide/essentials/component-basics). Не стесняйтесь пропустить его и вернуться позже.
