...
Возможно создать реактивный объект или массив с помощью функции [`reactive()`](/api/reactivity-core#reactive):
...
См. также: [Типизированная реактивность](/guide/typescript/composition-api#typing-reactive) <sup class="vt-badge ts" />
...
Чтобы дождаться завершения обновления DOM после изменения состояния, следует использовать функцию [nextTick()](/api/general#nexttick) глобального API:
...
Для устранения ограничений `reactive()`, Vue также предоставляет функцию [`ref()`](/api/reactivity-core#ref) которая позволяет создавать реактивные **"ref-ссылки"**, которые могут содержать значения любого типа:
...
См. также: [Типизированные ref-ссылки](/guide/typescript/composition-api#typing-ref) <sup class="vt-badge ts" />
