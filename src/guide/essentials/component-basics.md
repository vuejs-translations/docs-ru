...
![Дерево компонентов: корневой компонент и вложенные дочерние](./images/components.png)
...
export default {
  ...
  // Можно также указать шаблон в DOM:
...
Здесь шаблон вставляется в виде строки JavaScript, которую Vue скомпилирует на лету. Вы также можете использовать селектор ID, указывающий на элемент (обычно это встроенные элементы `<template>`) — Vue будет использовать его содержимое в качестве источника шаблона.
...
Если вы создаете свои шаблоны непосредственно в DOM (например, как содержимое встроенного элемента `<template>`), то шаблон будет подчиняться собственному поведению браузера при разборе HTML. В таких случаях необходимо использовать регистр `kebab-case` и явные закрывающие теги для компонентов:
...
Как можно увидеть выше, `<slot>` задаёт место подстановки контента — и это всё. Готово!
...
HTML теги и атрибуты не чувствительны к регистру, поэтому браузеры будут интерпретировать любые большие буквы как маленькие. Это означает, что имена компонентов из PascalCase, имена props из camelCase и имена `v-on` обработчиков событий должны быть преобразованы в kebab-case (разделённые дефисом) эквиваленты:
