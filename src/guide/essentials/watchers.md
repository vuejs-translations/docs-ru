...
В Vue 3.5+, the `deep` option can also be a number indicating the max traversal depth - i.e. how many levels should Vue traverse an object's nested properties.
...
Мы можем использовать [`onWatcherCleanup()`](/api/reactivity-core#onwatchercleanup) <sup class="vt-badge" data-text="3.5+" /> API для регистрации функции очистки, который будет вызываться, когда наблюдатель становится недействительным и собирается перезапуститься:
...
Обратите внимание, что `onWatcherCleanup` поддерживается только в Vue 3.5+ и должен вызываться во время синхронного вызова функции эффекта `watchEffect`  или обртного вызова `watch`: ты не сможешь его вызвать после `await` в асинхронной функции.

Альтернативно, в функция `onCleanup` передается в callback `watch` в качестве третьего аргумент, и в `watchEffect` в качестве первого аргумента:
