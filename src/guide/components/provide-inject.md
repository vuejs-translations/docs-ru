...
Обратите внимание, хотя компоненту `<Footer>` не нужны входные параметры, ему все равно необходимо объявить и передать их, чтобы `<DeepChild>` мог получить к ним доступ. Если есть более длинная родительская цепочка, по пути будет затронуто больше компонентов. Это называется "проп дриллинг", и с этим определенно не весело иметь дело.

В таких случаях можно использовать пару `provide` и `inject`. Родительские компоненты могут служить **провайдерами зависимостей** для всех своих потомков. Любой компонент в дереве-потомке, независимо от его глубины, может **ввести** зависимости, предоставляемые компонентами, расположенными выше в его родительской цепочке.
...
## Предоставление на уровне приложений {#app-level-provide}
...
app.provide(/* key */ 'message', /* value */ 'привет!')
```
...
Инъекции разрешаются **раньше**, чем собственное состояние компонента, поэтому к инжектированным свойствам можно обращаться в `data()`:
...
При использовании синтаксиса массива для `inject`, инжектируемые свойства отображаются в экземпляре компонента с использованием того же ключа. В приведенном примере свойство было предоставлено под ключом `"message"`, и инжектировано как `this.message`. Локальный ключ совпадает с ключом инъекции.

Если мы хотим инжектировать свойство с использованием другого локального ключа, то необходимо использовать объектный синтаксис для опции `inject`:
...
Функция `computed()` обычно используется в компонентах Composition API, но ее также можно использовать для дополнения определенных вариантов использования в Options API. Вы можете узнать больше о его использовании, прочитав [Основы реактивности](/guide/essentials/reactivity-fundamentals.html) и [Вычисляемые свойства](/guide/essentials/computed.html) с параметром API Preference, установленным на Composition API.
