...
## Слот содержимое и выход {#slot-content-and-outlet}
...
Элемент `<slot>`  - это **выход слота**, который указывает, где должно быть выведено **содержимое родительского слота**.
...
Благодаря использованию слотов наш `<FancyButton>` стал более гибким и многоразовым. Теперь мы можем использовать его в разных местах с разным внутренним содержимым, но с одинаковым стилем.
...
Мы можем захотеть, чтобы текст "Отправить" отображался внутри `<button>`, если родитель не предоставил никакого содержимого слота. Чтобы сделать "Отправить" резервным содержимым, мы можем поместить его между тегами `<slot>`:
...
На самом деле, это очень похоже на то, как компилируются слоты с областью видимости и как вы будете использовать слоты с областью видимости в [функциях ручного рендеринга](/guide/extras/render-function.html).
...
Обратите внимание, что `имя ` слота не будет включено в входной параметр, поскольку оно зарезервировано - таким образом, результирующий `headerProps` будет `{ message: 'hello' }`.
...
Хотя это интересный паттерн, большинство из того, что можно достичь с помощью компонентов без рендеринга, может быть достигнуто более эффективным способом с помощью Composition API, без накладных расходов на дополнительную вложенность компонентов. Позже мы увидим, как можно реализовать ту же функциональность отслеживания мыши с помощью [Составной](/guide/reusability/composables.html).
