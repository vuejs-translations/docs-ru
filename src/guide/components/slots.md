...
Благодаря слотам, `<FancyButton>` отвечает за отрисовку внешнего `<button>` (и ее причудливой стилизации), в то время как внутреннее содержимое предоставляется родительским компонентом.
...
Благодаря использованию слотов наш `<FancyButton>` стал более гибким и переиспользуемым. Теперь мы можем использовать его в разных местах с разным внутренним содержимым, но с одинаковой стилизацией.
...
Это приведет к отображению содержимого по умолчанию — "Отправить":
...
Обратите внимание, что выражение подчиняется [синтаксическим ограничениям](/guide/essentials/template-syntax.md#dynamic-argument-syntax-constraints) аргументов динамической директивы.
...
Пример использования `<FancyList>`, который мы рассмотрели выше, инкапсулирует как логику повторного использования (выборка данных, пагинация и т.д.), так и визуальный вывод, при этом делегируя часть визуального вывода компоненту-потребителю с помощью слотов с ограниченной областью видимости.
