...
Компонент может испускать пользовательские события непосредственно в выражениях шаблона (например, в обработчике `v-on`), используя встроенный метод `$emit`:
...
:::tip Совет
Все дополнительные аргументы, переданные в `$emit()` после имени события, будут переданы слушателю. Например, при `$emit('foo', 1, 2, 3)` функция слушатель получит три аргумента.
:::
...
Если вы используете явную функцию `setup` вместо `<script setup>` события должны быть объявлены с помощью опции [`emits`](/api/options-state.html#emits), а функция `emit` доступна в контексте `setup()`:
...
Если вы используете TypeScript с `<script setup>`, можно также объявить испускаемые события с помощью аннотаций чистого типа:
...
Подробнее: [Типизирование пользовательских событий компонента](/guide/typescript/composition-api.html#typing-component-emits) <sup class="vt-badge ts" />
...
См. также: [Типизирование пользовательских событий компонента](/guide/typescript/options-api.html#typing-component-emits) <sup class="vt-badge ts" />

</div>

Хотя это и необязательно, рекомендуется определить все генерируемые события, чтобы лучше документировать, как должен работать компонент. Это также позволяет Vue исключать известных слушателей из [аварийных атрибутов](/guide/components/attrs.html#v-on-listener-inheritance), избегая пограничных случаев, вызванных событиями DOM, которые вручную отправляются сторонним кодом.

:::tip Совет
Если в опции `emits` определено собственное событие (например, `click`), слушатель теперь будет прослушивать только события `click`, испускаемые компонентом, и больше не будет реагировать на собственные события `click`.
:::
...
## Использование `v-model` {#usage-with-v-model}

Пользовательские события также можно использовать для создания пользовательских входных данных, которые работают с `v-model`. Давайте рассмотрим, как `v-model` используется в нативном элементе:
...
2. При срабатывании нативного события `input`, испустить пользовательское событие `update:modelValue` с новым значением
...
[Попробовать в песочнице](https://sfc.vuejs.org/#)

</div>
<div class="composition-api">

[Попробовать в песочнице](https://sfc.vuejs.org/#)

</div>

Другой способ реализации `v-model` в этом компоненте - использовать записываемое `computed` свойство с геттером и сеттером. Метод `get` должен возвращать свойство `modelValue`, а метод `set` вызывать соответствующее событие:
