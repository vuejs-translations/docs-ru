...
Изучая работу с формами, можно заметить, что у `v-model` есть [встроенные модификаторы](/guide/essentials/forms#modifiers) - `.trim`, `.number` и `.lazy`. Но в некоторых случаях может потребоваться добавить пользовательские модификаторы.
