...
Если на одном компоненте стоят и явные привязки, и `v-bind`, Vue объединяет их через `mergeProps()`. Правила зависят от типа ключа:

- **Обычные входные параметры** — остаётся последнее значение:
...
:::tip
Все правила объединения — в справочнике API [`mergeProps()`](/api/render-function#mergeprops).
:::
