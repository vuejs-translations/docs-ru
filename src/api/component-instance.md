...
Объект, возвращённый из параметра [`data`](./options-state#data), который компонент сделал реактивным. Экземпляр компонента проксирует доступ к свойствам объекта данных.
...
- **Подробности**
  ...
  Будут включены только параметры, объявленные с помощью опции [`props`](./options-state#props). Экземпляр компонента предоставляет прокси-доступ к свойствам своего объекта props.
...
- **Подробности**
  ...
  :::tip Совет
  Для обеспечения согласованности рекомендуется использовать [ссылки на шаблон](/guide/essentials/template-refs) для прямого доступа к элементам вместо того, чтобы полагаться на `$el`.
  :::
...
- **См. также** [`app.config.optionMergeStrategies`](/api/application#app-config-optionmergestrategies)
...
Объект, представляющий [слоты](/guide/components/slots), передаваемый родительским компонентом.
...
- **Подробности**
  ...
  Обычно используется при ручном создании [функций рендеринга](/guide/extras/render-function), но также может использоваться для определения наличия слота.
  ...
  Если слот является [слотом с ограниченной областью действия](/guide/components/slots#scoped-slots), аргументы, переданные функциям слота, доступны слоту в качестве входных параметров слота.

- **См. также** [Функции рендеринга — слоты рендеринга](/guide/extras/render-function#rendering-slots)
...
- **Подробности**
  ...
  [Атрибуты Fallthrough](/guide/components/attrs) это атрибуты и обработчики событий, передаваемые родительским компонентом, но не объявленные дочерним компонентом как prop или событие, передаваемое дочерним компонентом.

  По умолчанию все, что содержится в `$attrs`, будет автоматически унаследовано в корневом элементе компонента, если существует только один корневой элемент. Это поведение отключено, если компонент имеет несколько корневых узлов, и может быть явно отключено с помощью опции [`inheritAttrs`](./options-misc#inheritattrs).

- **См. также**

  - [Атрибуты Fallthrough](/guide/components/attrs)
...
- **Подробности**
  ...
  Первый аргумент — это источник наблюдения. Это может быть строка имени свойства компонента, простая строка пути, разделенная точкой, или [getter функция](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/get#description).

  Второй аргумент — это коллбэк. Обратный вызов получает новое значение и старое значение наблюдаемого источника.

  - **`immediate`**: коллбэк запускается сразу после создания наблюдателя. Старое значение будет `undefined` при первом вызове.
  - **`deep`**: принудительный глубокий обход источника, если это объект, так что обратный вызов срабатывает при глубоких мутациях. Смотрите [Глубокие наблюдатели](/guide/essentials/watchers#deep-watchers).
  - **`flush`**: настройте время очистки коллбэка. См. раздел [Тайминг очистки коллбэка](/guide/essentials/watchers#callback-flush-timing) и [`watchEffect()`](/api/reactivity-core#watcheffect).
  - **`onTrack / onTrigger`**: Для отладки зависимостей наблюдателя. См. [Отладка наблюдателя](/guide/extras/reactivity-in-depth#watcher-debugging).
...
- **См. также**
  ...
  - [Компонент - События](/guide/components/events)
  - [`emits` опции](./options-state#emits)
...
Откладывает вызов коллбэка до следующего цикла обновления DOM. Используйте его сразу после изменения данных, чтобы дождаться обновления DOM. [`nextTick()`](./general#nexttick).
...
- **См. также** [`nextTick()`](./general#nexttick)
