...
Принимает [геттер функцию](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/set#description) и возвращает иммутабельный (доступен только для чтения) реактивный [ref-объект](#ref) для возвращаемого значения из геттера. Также можно вернуть объект используя функции `get` и `set` для создания ref-объекта с возможностью записи.
