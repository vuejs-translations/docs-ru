...
- **Подробности**
  ...
  - Объект, ключами которого являются имена локальных свойств для привязки, а значениями:
    ...
    - Объектом, в котором:
      ...
      - В качестве запасного значения используется свойство `default`. Аналогично значениям по умолчанию для входных параметров, для типов объектов необходимо использовать функцию-фабрику, чтобы избежать совместного использования значений несколькими экземплярами компонентов.
