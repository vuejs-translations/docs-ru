...
- При использовании объявления при помощи типов эквивалентное объявление во время исполнения кода автоматически генерируется на основе статического анализа, что устраняет необходимость в двойном объявлении и обеспечивает корректное поведение во время выполнения кода.

  - В режиме разработки компилятор попытается вывести из типов соответствующую проверку во время выполнения. Например здесь `foo: String` выведется из типа `foo: string`. Импортированные типы также разрешаются при условии, что TypeScript установлен как peer-зависимость Vue.
...
- В версии 3.2 и ниже дженерик для `defineProps()` был ограничен литератом типа или ссылкой на локальный интерфейс.
  ...
  Это ограничение было снято в версии 3.3. Последняя версия Vue поддерживает возможность ссылки на импортированные и ограниченный набор сложных типов в позиции параметра типа. Однако, так как преобразование типов во время выполнения все еще основано на AST, некоторые сложные типы, требующие фактического анализа, например, условные типы, не поддерживаются. Вы можете использовать условные типы для типизации одного входного параметра, но не для цельного объекта входных параметров.
...
## Операторы импорта {#imports-statements}

Операторы импорта во Vue соответствуют [спецификации модулей ECMAScript](https://nodejs.org/api/esm.html).
Кроме того, можно использовать алиасы, определённые в конфигурации сборщика:
...
Директиву `@vue-generic` можно использовать для передачи явных типов, когда тип не удаётся вывести:
...
Чтобы использовать ссылку на дженерик-компонент в `ref`, нужна библиотека [`vue-component-type-helpers`](https://www.npmjs.com/package/vue-component-type-helpers), так как `InstanceType` в этом случае не подойдёт.
