...
- **Подробности**
  ...
  Этот хук обычно используется для выполнения коллбэков, которым необходим доступ к отрисованному DOM компонента, или для ограничения кода, связанного с DOM, на клиенте в [серверно-отрисованном приложении](/guide/scaling-up/ssr).
...
Регистрирует коллбэк, который будет вызван после того, как компонент обновит своё дерево DOM в результате реактивного изменения состояния.
...
- **Подробности**
  ...
  Этот хук вызывается после любого обновления DOM компонента, которое может быть вызвано различными изменениями состояния. Если вам нужно получить доступ к обновленному DOM после определенного изменения состояния, используйте [nextTick()](/api/general#nexttick).
...
Регистрирует хук для вызова непосредственно перед тем, как компонент собирается обновить своё дерево DOM из-за реактивного изменения состояния.
...
- **Тип**
  ...
  - **Подробности**
    ...
    :::tip
    В продакшене 3-й аргумент (`info`) будет представлять собой сокращенный код вместо полной информационной строки. Сопоставление кода и строки можно найти в [Руководстве по кодам ошибок](/error-reference/#runtime-errors).
    :::
    ...
    **Правила распространения ошибок**
    ...
    - По умолчанию все ошибки по-прежнему отправляются на уровень приложения [`app.config.errorHandler`](/api/application#app-config-errorhandler), если он объявлен, чтобы об этих ошибках можно было сообщать сервису аналитики в одном месте.
...
Регистрирует коллбэк, который будет вызван после того, как экземпляр компонента будет вставлен в DOM как часть дерева, кэшируемого с помощью [`<KeepAlive>`](/api/built-in-components#keepalive).
...
Регистрирует коллбэк, который будет вызван после удаления экземпляра компонента из DOM как части дерева, кэшируемого с помощью [`<KeepAlive>`](/api/built-in-components#keepalive).
...
- **См. также** [Рендеринг на стороне сервера](/guide/scaling-up/ssr)
