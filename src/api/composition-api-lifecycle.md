...
- **Подробности:**
  ...
  Хук получает три аргумента: ошибку, экземпляр компонента, вызвавший ошибку, и информационную строку, указывающую тип источника ошибки.
  ...
  **Правила распространения ошибок**.
  ...
  - По умолчанию все ошибки по-прежнему отправляются на уровень приложения [`app.config.errorHandler`](/api/application.html#app-config-errorhandler), если он определен, так что эти ошибки могут быть сообщены аналитической службе в одном месте.
...
Регистрирует ловушку отладки, которая будет вызываться, когда реактивная зависимость отслеживается эффектом рендеринга компонента.
...
- **Подробности:**
  ...
  Если коллбэк возвращает обещание, серверный рендерер будет ждать, пока обещание не будет разрешено, прежде чем приступить к рендерингу компонента.
...
- **Пример:**
  ...
  onServerPrefetch(async () => {
    ...
    // pre-fetch данных на сервере, так как это быстрее, чем на клиенте
