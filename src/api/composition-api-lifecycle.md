...
- **Подробности**
  ...
  Этот хук вызывается после любого обновления DOM компонента, которое может быть вызвано различными изменениями состояния. Если вам нужно получить доступ к обновлённому DOM после определенного изменения состояния, используйте [nextTick()](/api/general#nexttick).
...
- **Тип**
  ...
  - **Подробности**
    ...
    :::tip Совет
    В продакшене 3-й аргумент (`info`) будет представлять собой сокращённый код вместо полной информационной строки. Сопоставление кода и строки можно найти в [Руководстве по кодам ошибок](/error-reference/#runtime-errors).
    :::

    Вы можете изменить состояние компонента в `onErrorCaptured()`, чтобы отобразить состояние ошибки пользователю. Однако важно, чтобы состояние ошибки не отображало исходное содержимое, вызвавшее ошибку - в противном случае компонент будет брошен в бесконечный цикл рендеринга.
